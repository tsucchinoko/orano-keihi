...
pub async fn test_r2_connection_secure() -> Result<bool, String> {
    ...
    let security_manager = SecurityManager::new();
    security_manager.log_security_event("r2_connection_test_requested", "フロントエンドからの接続テスト要求");
    
    // R2設定を取得
    let config = crate::services::config::R2Config::from_env()
        .map_err(|e| {
    ...
    // R2クライアントを作成
    let client = crate::services::r2_client::R2Client::new(config).await
        .map_err(|e| {
...
pub async fn get_environment_info() -> Result<HashMap<String, String>, String> {
    ...
    info.insert("is_production".to_string(), env_config.is_production().to_string());
    info.insert("is_development".to_string(), env_config.is_development().to_string());
...
pub async fn log_security_event(event_type: String, details: String) -> Result<(), String> {
    info!("セキュリティイベントのログ記録要求: type={}, details={}", event_type, details);
...
mod tests {
    ...
    async fn test_log_security_event() {
        let result = log_security_event(
            "test_event".to_string(),
            "テストイベントの詳細".to_string()
        ).await;
        assert!(result.is_ok());
    }
