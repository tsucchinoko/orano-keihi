<script lang="ts">
                    ...
                    import("@tauri-apps/api/core").then(
                        ({ convertFileSrc }) => {
                            if (
                                ...
                                !subscription.receipt_path.startsWith(
                                    "https://",
                                )
                            ) {
                                receiptPreview = convertFileSrc(
                                    subscription.receipt_path,
                                );
                            }
                        },
    ...
    async function deleteReceipt() {
        ...
        try {
            console.info(
                `🗑️ サブスクリプションの領収書削除開始: subscription_id=${subscription.id}, receipt_path=${subscription.receipt_path}`,
            );

            // R2から領収書を削除（deleteSubscriptionReceiptFromR2関数を使用）
            const r2DeleteResult = await deleteSubscriptionReceiptFromR2(
                subscription.receipt_path,
            );
            ...
            console.info(
                `🗑️ DB更新開始: subscription_id=${subscription.id}, 専用削除関数を使用`,
            );

            const dbDeleteResult = await deleteSubscriptionReceipt(
                subscription.id,
            );
            ...
            console.info(
                `🗑️ サブスクリプションの領収書削除完了: subscription_id=${subscription.id}`,
            );
        } catch (error) {
        ...
        try {
            ...
            if (subscription) {
                ...
                if (success && expenseStore.subscriptions.length > 0) {
                    const lastSubscription =
                        expenseStore.subscriptions[
                            expenseStore.subscriptions.length - 1
                        ];
            ...
            if (receiptFile && savedSubscriptionId) {
                ...
                if (uploadResult.error) {
                    toastStore.error(
                        `領収書のアップロードに失敗しました: ${uploadResult.error}`,
                    );
        ...
        <div>
            ...
            {#if billingCycle === "annual" && monthlyAmount() > 0}
                <p class="text-sm text-gray-600 mt-2">
                    月額換算: ¥{monthlyAmount().toLocaleString("ja-JP", {
