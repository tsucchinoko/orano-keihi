...
class ExpenseStore {
	...
	async loadSubscriptions(activeOnly: boolean = false): Promise<void> {
		...
		try {
			...
			this.error = `サブスクリプションの読み込みに失敗しました: ${err}`;
		} finally {
			this.isLoading = false;
		}
	}
	...
	async addSubscription(
		...
		try {
			...
			this.error = `サブスクリプションの作成に失敗しました: ${err}`;
			return false;
		} finally {
			this.isLoading = false;
		}
	...
	async modifySubscription(
		...
		try {
			...
			this.error = `サブスクリプションの更新に失敗しました: ${err}`;
			return false;
		} finally {
			this.isLoading = false;
		}
	...
	async toggleSubscription(id: number): Promise<boolean> {
		...
		try {
			...
			this.error = `サブスクリプションの状態切り替えに失敗しました: ${err}`;
			return false;
		} finally {
			this.isLoading = false;
		}
	...
	async loadMonthlySubscriptionTotal(): Promise<void> {
		try {
			...
			this.error = `月額合計の読み込みに失敗しました: ${err}`;
		}
	}
